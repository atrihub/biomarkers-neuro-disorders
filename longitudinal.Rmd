...
fit_lme_int_ml <- update(fit_lme_int, method='ML')
fit_lme_ml <- update(fit_lme, method="ML")
anova(fit_lme_int_ml, fit_lme_ml)
```
...
* We have discussed linear mixed effect models (LMEs) which treats time as a continuous variable
...
.large[
* A popular model for clinical trials
* Makes no (or very general) assumptions about the mean and variance/covariance structure.
...
+ See `r Citet(bib=bib, 'donohue2023natural')` for applications to Alzheimer's trials
]
...
## Modeled group contrasts for categorical, quadratic, and natural cubic spline models
...
hipp_fit <- lme(Hippocampus ~ yrs*DX.bl + AGE + PTGENDER + ICV.bl,
  hippvol, random = ~yrs|RID, method='ML')

hipp_fit_site <- lme(Hippocampus ~ yrs*DX.bl + AGE + PTGENDER + ICV.bl,
  hippvol, random = list(SITE = ~ yrs, RID = ~ yrs), method='ML')
